        ...
        if self.strategy == 'label':
            for col, le in self.label_encoders.items():
                if col in df_out.columns:
                    # Preserve original by creating a new encoded column or just keeping both.
                    # To keep it consistent with one-hot (which creates new names), 
                    # we will keep the original and create 'col_encoded'.
                    # However, to avoid breaking model feature lists, we'll keep 'col' as the encoded one
                    # and save 'col_raw' as the original.
                    df_out[f"{col}_raw"] = df_out[col].copy()
            ...
            # Concatenate dummies but DO NOT drop the original columns
