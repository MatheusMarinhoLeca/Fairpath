...
def save_plot(fig, filename, output_dir="outputs/reports/assets"):
    ...
    plt.close(fig)
    plt.close('all') # Force complete cleanup of all figures
    return path
...
def plot_box_plots(df, numeric_cols, filename):
    ...
    for i, col in enumerate(plot_cols):
        sns.boxplot(data=df, y=col, ax=axes[i], orient='v')
...
def plot_violin_plots(df, numeric_cols, target_col, filename):
    ...
    for i, col in enumerate(plot_cols):
        sns.violinplot(data=df, x=target_col, y=col, ax=axes[i], orient='v')
